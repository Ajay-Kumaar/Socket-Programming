...
void handle_player(int client_socket, int player_id)
{
    ...
    if (bytes_received > 0)
	{
        ...
        *key_end = '\0';
		...
		encode_and_send(message, strlen(message), client_sockets[0]);

		bzero(message, sizeof(message));
		snprintf(message, sizeof(message), "Opponent's turn");
		encode_and_send(message, strlen(message), client_sockets[1]);
    }
	while (1)
	{
            ...
            if (payload_length > 0)
			{
                ...
                if( row >= 0 && row < 3 && col >= 0 && col < 3 && board[row][col] == ' ')
				{
					...
					else
					{
						...
						bzero(message, sizeof(message));
						snprintf(message, sizeof(message), "Your turn");
						encode_and_send(message, strlen(message), client_sockets[1-p]);

						bzero(message, sizeof(message));
						snprintf(message, sizeof(message), "Opponent's turn");
						encode_and_send(message, strlen(message), client_sockets[p]);
